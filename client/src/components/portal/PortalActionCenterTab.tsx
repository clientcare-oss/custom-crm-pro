  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="relative w-full md:w-64">
          ...
          <Input
            ...
            className="pl-8 text-xs bg-slate-900/60 border-white/10 text-white placeholder:text-white/30 rounded-xl h-9 focus:border-amber-400/50"
          />
        </div>
      </div>
      ...
      {filteredItems.length === 0 ? (
        <div className="text-center py-16 px-4 rounded-2xl border border-dashed border-white/10 bg-slate-900/30 space-y-3">
            ...
            return (
              <Card
                ...
                className={`group relative rounded-2xl border p-5 transition-all duration-200 flex flex-col justify-between ${
                  isAttention
                    ? "bg-gradient-to-b from-[#111f38] to-[#0a1424] border-amber-400/40 shadow-[0_4px_20px_rgba(245,181,68,0.08)] hover:border-amber-400/70"
                    : item.isCompleted
                    ? "bg-slate-900/40 border-white/10 hover:border-white/20"
                    : "bg-[#0c182c]/80 border-white/10 hover:border-white/20"
                }`}
              >
                <div>
      ...
      <Dialog open={isActionModalOpen} onOpenChange={setIsActionModalOpen}>
        <DialogContent className="max-w-2xl bg-[#0a1526] border-white/15 text-white rounded-2xl p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
          {selectedAction && (
      ...
      <Dialog open={isStartActionOpen} onOpenChange={setIsStartActionOpen}>
        <DialogContent className="max-w-xl bg-[#0a1526] border-white/15 text-white rounded-2xl p-6 shadow-2xl">
          <DialogHeader>
