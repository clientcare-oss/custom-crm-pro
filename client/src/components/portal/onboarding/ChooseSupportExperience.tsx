...
export function ChooseSupportExperience({
  ...
  const totalPrice = currentTierObj.price;
    ...
    <div className="p-6 md:p-8 lg:p-10 space-y-8 max-w-4xl mx-auto animate-in fade-in duration-300">
      ...
      <div className="border-b border-white/10 pb-6">
        ...
        <div className="flex items-center gap-2 mt-6">
          {[
            { num: 1, label: "Support Tier" },
            { num: 2, label: "Review & Schedule" },
            ...
            <div key={s.num} className="flex items-center gap-2 flex-1">
              ...
              {s.num < 3 && <div className="h-0.5 flex-1 bg-white/10 mx-1" />}
            </div>
          ))}
        </div>
      </div>

      {/* Step 1: Select Support Level */}
      {step === 1 && (
        <div className="space-y-6">
          <div>
            <h2 className="text-lg font-bold text-white">Select Your Representation Tier</h2>
            <p className="text-xs text-white/60 mt-0.5">All packages include dedicated direct communication and document access.</p>
          </div>
                ...
                >
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <div className="text-2xl font-black text-amber-300 mt-2">${t.price.toLocaleString()}</div>
                    </div>
      ...
      {/* Step 2: Review Order & Billing Frequency */}
      {step === 2 && (
        <div className="space-y-6">
          <div>
            <h2 className="text-lg font-bold text-white">Review Your Advocacy Selection</h2>
            <p className="text-xs text-white/60 mt-0.5">Confirm selection details and choose your payment schedule.</p>
          </div>
          ...
          <Card className="border-white/10 bg-white/[0.02] p-6 space-y-4">
            <div className="flex items-center justify-between border-b border-white/10 pb-4">
              <div>
                ...
                <p className="text-xs text-white/60">Advocacy Representation & Case Management</p>
              </div>
              <div className="text-xl font-bold text-amber-300">${totalPrice.toLocaleString()}</div>
            </div>
            ...
            <div className="space-y-2 pt-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                >
                  <p className="font-bold text-white text-sm">Pay in Full (${totalPrice.toLocaleString()})</p>
                ...
                >
                  <p className="font-bold text-white text-sm">2 Installments (${installmentPrice.toLocaleString()} today)</p>
          ...
          <Card className="border-amber-400/30 bg-gradient-to-br from-amber-400/5 to-white/[0.02] p-6 space-y-5">
            <div className="flex items-center justify-between border-b border-white/10 pb-4 text-xs">
              ...
              <span className="text-xl font-bold text-amber-300">
                ${paymentPlan === "full" ? totalPrice.toLocaleString() : installmentPrice.toLocaleString()}
              </span>
            </div>
            ...
            >
              ...
              {isProcessing ? "Authorizing Payment..." : `Authorize & Pay $${paymentPlan === "full" ? totalPrice.toLocaleString() : installmentPrice.toLocaleString()}`}
            </Button>
          </Card>
